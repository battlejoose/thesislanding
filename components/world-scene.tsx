...
export default function WorldScene({theme,motion}:{theme:Theme;motion:boolean}){
  ...
  return <div className="scene-stage" data-ready={ready} data-motion={motion}>
    {!ready&&<div className="scene-reveal"><img src={`/art/${theme}.webp`} alt="" width={1280} height={853}/>{!failed&&<span className="scene-loading"><i/><i/><i/></span>}</div>}
    ...
    <div className="scene-ui"><span><Box size={13}/>{failed?'STILL WORLD':'A LIVING 3D WORLD'}</span>{!failed&&<span><MoveHorizontal size={16}/>Drag to explore</span>}</div>
  </div>;
}
