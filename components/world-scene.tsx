...
export default function WorldScene({motion,webgl=true,onReady}:{motion:boolean;webgl?:boolean;onReady?:()=>void}){
  ...
  // The parent memoises onReady, so reading it inside the effect is stable.
  ...
  useEffect(()=>{
    ...
    if(!webgl){setFailed(true);settle();return;}
    let disposed=false;setReady(false);setFailed(false);
    const fail=()=>{setFailed(true);setReady(false);settle();};container.addEventListener('scene-error',fail);
    void import('@/lib/three-world').then(({createWorld})=>{if(disposed)return;engine.current=createWorld(container,()=>{setReady(true);settle();});engine.current.setMotion(motionRef.current);}).catch(()=>{if(!disposed){setFailed(true);settle();}});
