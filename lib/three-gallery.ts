...
const TAU=Math.PI*2;
const palette={roots:{side:'#453826',paper:'#e7ebd4',ink:'#273c2c',trim:'#789052'},brooklyn:{side:'#653c30',paper:'#f0e6cf',ink:'#302922',trim:'#b19a75'},steampunk:{side:'#343130',paper:'#dfcaa2',ink:'#49351f',trim:'#c29656'}};
...
interface Tile {scene:T.Scene;camera:T.PerspectiveCamera;group:T.Group;article:HTMLElement;project:Project;imageMaterial:T.MeshBasicMaterial;imageTexture:T.Texture;videoTexture:T.VideoTexture|null;video:HTMLVideoElement|null;statusTexture:T.Texture;statusMaterial:T.MeshBasicMaterial;status:string;animate:Array<(t:number)=>void>;hover:boolean;rx:number;ry:number;baseY:number;}
function makeTile(article:HTMLElement,p:Project,index:number,theme:Theme,font:Font,wake:()=>void):Tile{
  const colors=palette[theme],scene=new T.Scene(),camera=new T.PerspectiveCamera(42,1,.1,30);camera.position.set(0,.12,10.4);camera.lookAt(0,.12,0);
  ...
  const imageMaterial=new T.MeshBasicMaterial({map:imageTexture});mesh(group,new T.PlaneGeometry(4.35,2.57),imageMaterial,0,1.27,.585);
  ...
  });printed(group,info,4.35,2.59,0,-1.42,.588);
  ...
  const statusTexture=makeStatus('▷  DISCOVER '+p.name.toUpperCase());const statusMaterial=new T.MeshBasicMaterial({map:statusTexture,transparent:true});mesh(group,new T.PlaneGeometry(4.18,.435),statusMaterial,0,.232,.625);
  ...
  const shadowTex=canvasTexture(128,128,ctx=>{const grad=ctx.createRadialGradient(64,64,10,64,64,64);grad.addColorStop(0,'#0008');grad.addColorStop(1,'#0000');ctx.fillStyle=grad;ctx.fillRect(0,0,128,128);});const shadow=printed(scene,shadowTex,6.4,7.5,.2,-.26,-1.1);shadow.castShadow=false;
  const baseY=(index%2?1:-1)*.27;group.rotation.set(.09,baseY,0);
  return {scene,camera,group,article,project:p,imageMaterial,imageTexture,videoTexture:null,video:null,statusTexture,statusMaterial,status:'',animate,hover:false,rx:0,ry:0,baseY};
}
export async function createGallery(container:HTMLDivElement,articles:HTMLElement[],projects:Project[],theme:Theme,onReady:()=>void){
  ...
  function render(now:number){frame=0;if(disposed||document.hidden)return;const dt=Math.min((now-last)/1000,.04);last=now;if(motion)elapsed+=dt;const width=window.innerWidth,height=window.innerHeight;const rects=tiles.map(t=>t.article.getBoundingClientRect());renderer.setScissorTest(false);renderer.clear();renderer.setScissorTest(true);visibleCount=0;let activeVideo=false,settling=false;
    tiles.forEach((tile,index)=>{const r=rects[index];if(r.bottom<0||r.top>height||r.right<0||r.left>width)return;visibleCount++;
      ...
      if(state!==tile.status){tile.status=state;tile.statusTexture.dispose();tile.statusTexture=canvasTexture(768,80,ctx=>{ctx.fillStyle='#14271eee';ctx.fillRect(0,0,768,80);ctx.fillStyle='#eaf0df';ctx.font='25px sans-serif';ctx.fillText(state,24,53);ctx.textAlign='right';ctx.fillText('↗',740,53);});tile.statusMaterial.map=tile.statusTexture;tile.statusMaterial.needsUpdate=true;}
      const tx=.085+(motion?Math.sin(elapsed*.5+index)*.018+tile.rx:0),ty=tile.baseY+(motion?Math.sin(elapsed*.42+index*.9)*.035+tile.ry:0);
      const selected=tile.article.dataset.selected==='true';const z=selected?.32:tile.hover?.18:0;
      ...
      tile.group.rotation.x+=(tx-tile.group.rotation.x)*.1;tile.group.rotation.y+=(ty-tile.group.rotation.y)*.1;tile.group.position.z+=(z-tile.group.position.z)*.1;tile.group.position.y=motion?Math.sin(elapsed*.8+index*.8)*.07:0;tile.animate.forEach(fn=>fn(elapsed));
      tile.camera.aspect=r.width/r.height;tile.camera.updateProjectionMatrix();renderer.setViewport(r.left,height-r.bottom,r.width,r.height);renderer.setScissor(Math.max(0,r.left),Math.max(0,height-r.bottom),Math.min(r.right,width)-Math.max(0,r.left),Math.min(r.bottom,height)-Math.max(0,r.top));renderer.render(tile.scene,tile.camera);
    });
